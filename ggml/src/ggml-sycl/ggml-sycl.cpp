...
static bool reorder_qw_q4_k_xmx(
    ...
    sycl_reorder_temp_buffer tmp(stream, size);
    if (!tmp) {
        GGML_LOG_WARN("%s: failed to allocate %zu bytes, skipping XMX reorder\n", __func__, size);
        return false;
    }
    ...
    uint8_t * qs = data_device, * scales = qs + total_tb * 1024;
    auto * dm = (sycl::half *) (scales + total_tb * 96);
    const int nblocks = nrows * blocks;
    const uint8_t * source_qs = input;
    const uint8_t * source_scales = input + nblocks * 128;
    const auto * source_dm = (const sycl::half2 *) (source_scales + nblocks * 12);
    auto event = stream->parallel_for(total_tb, [=](auto index) {
        ...
        for (int g = 0; g < 8; ++g) {
            for (int lane = 0; lane < 16; ++lane) {
                for (int r = 0; r < 8; ++r) {
                    const int wi = (rt * 8 + r) * blocks + b;
                    const uint8_t * p = source_soa ? source_qs + wi * 128 + (g / 2) * 32 : source[wi].qs + (g / 2) * 32;
        ...
        for (int r = 0; r < 8; ++r) {
            const int wi = (rt * 8 + r) * blocks + b;
            const sycl::half2 value = source_soa ? source_dm[wi] : source[wi].dm;
            dm[tb * 16 + r] = value[0];
            dm[tb * 16 + 8 + r] = value[1];
        }
    });
...
static bool restore_qw_q4_k_xmx_to_soa(
    ...
    sycl_reorder_temp_buffer tmp(stream, size);
    if (!tmp) {
        GGML_LOG_WARN("%s: failed to allocate %zu bytes; retaining XMX layout\n", __func__, size);
        return false;
    }
    auto * input = static_cast<uint8_t *>(tmp.ptr);
    sycl::event copy_event;
    SYCL_CHECK(CHECK_TRY_ERROR(copy_event = stream->memcpy(input, data_device, size)));
    if (!g_ggml_sycl_use_async_mem_op) copy_event.wait();
    const uint8_t * in_qs = input, * in_scales = in_qs + total_tb * 1024;
    const auto * in_dm = (const sycl::half *) (in_scales + total_tb * 96);
    uint8_t * out_qs = data_device, * out_scales = out_qs + nblocks * 128;
    auto * out_dm = (sycl::half2 *) (out_scales + nblocks * 12);
    auto event = stream->parallel_for(nblocks, [=](auto index) {
        ...
        for (int j = 0; j < 128; ++j) {
            const int pair = j / 32, pos = j % 32, lane = pos & 15;
            const bool high = pos >= 16;
            const uint8_t pe = in_qs[tb * 1024 + ((2 * pair) * 16 + lane) * 8 + r];
            const uint8_t po = in_qs[tb * 1024 + ((2 * pair + 1) * 16 + lane) * 8 + r];
            const uint8_t qe = high ? pe >> 4 : pe & 15, qo = high ? po >> 4 : po & 15;
            out_qs[wi * 128 + j] = qe | (qo << 4);
        }
    ...
    stream->parallel_for(total_tb, [=](auto index) {
        ...
        for (int g = 0; g < 8; ++g) {
            for (int lane = 0; lane < 16; ++lane) {
                for (int r = 0; r < 8; ++r) {
                    const int wi = (rt * 8 + r) * blocks + b;
                    const uint8_t * p = source_soa ? soa_qs + wi * 128 + (g / 2) * 32 : source[wi].qs + (g / 2) * 32;
                    const uint8_t q0 = (g & 1) ? p[lane] >> 4 : p[lane] & 15, q1 = (g & 1) ? p[lane + 16] >> 4 : p[lane + 16] & 15;
        ...
        for (int r = 0; r < 8; ++r) {
            ...
            out_dm[tb * 16 + r] = value[0];
            out_dm[tb * 16 + 8 + r] = value[1];
        }
    });
}
...
static void opt_for_q4_xmx_gemm(ggml_backend_sycl_context & ctx, const ggml_tensor * src0, const ggml_tensor * src1) {
    ...
    if (reorder_qw_q4_k_xmx(static_cast<uint8_t *>(src0->data), src0->ne[0], src0->ne[1], ggml_nbytes(src0), source_soa, ctx.stream())) {
        ...
        extra->optimized_feature.q4_xmx = true;
    }
}
...
static void restore_q4_xmx_for_small_n(ggml_backend_sycl_context & ctx, const ggml_tensor * src0, const ggml_tensor * src1) {
    ...
    if (restore_qw_q4_k_xmx_to_soa(static_cast<uint8_t *>(src0->data), src0->ne[0], src0->ne[1], ggml_nbytes(src0), ctx.stream())) {
        extra->optimized_feature.q4_xmx = false;
...
static bool ensure_q5_xmx_layout(ggml_backend_sycl_context & ctx, const ggml_tensor * src0) {
    ...
    if (extra->optimized_feature.q5_xmx) return true;
    ggml_sycl_pool_alloc<uint8_t> input(ctx.pool(), ggml_nbytes(src0));
    ...
    extra->optimized_feature.reorder = false;
    extra->optimized_feature.q5_xmx = true;
    return true;
}
...
static bool ensure_q6_xmx_layout(ggml_backend_sycl_context & ctx, const ggml_tensor * src0) {
    auto * extra = static_cast<ggml_tensor_extra_gpu *>(src0->extra);
    if (!extra) return false;
    if (extra->optimized_feature.q6_xmx) return true;
    ggml_sycl_pool_alloc<uint8_t> input(ctx.pool(), ggml_nbytes(src0));
    if (!input.get()) return false;
    ctx.stream()->memcpy(input.get(), src0->data, ggml_nbytes(src0));
    pack_qw_q6_k_xmx(input.get(), static_cast<uint8_t *>(src0->data), src0->ne[0], src0->ne[1],
                     extra->optimized_feature.reorder, ctx.stream());
    extra->optimized_feature.reorder = false;
    extra->optimized_feature.q6_xmx = true;
    return true;
}
...
static void restore_q5_q6_xmx_outside_gemm(
    ...
    if (src0->type == GGML_TYPE_Q5_K && extra->optimized_feature.q5_xmx) {
        ...
        extra->optimized_feature.q5_xmx = false;
        extra->optimized_feature.reorder = true;
        ...
        extra->optimized_feature.q6_xmx = false;
        extra->optimized_feature.reorder = true;
    }
}
...
static bool try_mul_mat_q5_K_gemm_xmx(
    ...
    ggml_sycl_mul_mat_q5_K_gemm_xmx(ctx, src0, src1, dst);
    return true;
}
...
static bool try_mul_mat_q6_K_gemm_xmx(
    ...
    ggml_sycl_mul_mat_q6_K_gemm_xmx(ctx, src0, src1, dst);
    return true;
}
