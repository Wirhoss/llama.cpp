...
struct xmx_opencl_kernel_entry {
    ...
    xmx_executable_bundle bundle;
    bool has_kernels = false;
    sycl::kernel kernel_q4_small;
    sycl::kernel kernel_q5_small;
    sycl::kernel kernel_q6_small;
    sycl::kernel kernel_q6_n64;
    bool failed = false;
};
...
struct xmx_gemm_kernel_entry {
    ...
    bool has_kernels = false;
    sycl::kernel kernel;
    sycl::kernel q5_kernel;
    sycl::kernel q6_kernel;
    sycl::kernel dequant_kernel;
    sycl::kernel q5_dequant_kernel;
    sycl::kernel q6_dequant_kernel;
    bool failed = false;
};
...
bool xmx_ensure_gemm_kernels(ggml_backend_sycl_context & ctx) {
    ...
    if (entry.has_context && entry.context != context) {
        xmx_gemm_kernel_ready[ctx.device].store(false, std::memory_order_release);
        entry = {};
    }
    if (entry.has_kernels) {
    ...
    try {
        ...
        std::string log;
        auto eb = syclex::build(sb, std::vector<sycl::device>{device}, syclex::properties{syclex::save_log{&log}});
        if (!log.empty()) GGML_LOG_DEBUG("SYCL XMX Q4_K/Q5_K/Q6_K GEMM OpenCL build log: %s\n", log.c_str());
        entry.context = context;
        entry.has_context = true;
        entry.bundle = std::move(eb);
        entry.has_bundle = true;
        entry.kernel = entry.bundle->ext_oneapi_get_kernel("opencl_q4k_f16_gemm_m32n64");
        ...
        entry.failed = true;
        GGML_LOG_WARN("SYCL XMX Q4_K/Q5_K/Q6_K GEMM unavailable, using fallback: %s\n", e.what());
        return false;
    }
}
...
void ggml_sycl_mul_mat_q4_K_gemm_xmx(
    ...
    ctx.stream()->submit([&](sycl::handler & h) {
        h.set_arg(0, src0->data); h.set_arg(1, activation.get()); h.set_arg(2, dst->data);
...
void ggml_sycl_dequantize_q4_K_xmx_to_f16(
    ...
    const size_t groups = static_cast<size_t>(m / 8) * blocks;
    const sycl::kernel & kernel = xmx_gemm_kernels[ctx.device].dequant_kernel;
    ctx.stream()->submit([&](sycl::handler & h) {
        h.set_arg(0, weights); h.set_arg(1, output); h.set_arg(2, m); h.set_arg(3, k); h.set_arg(4, blocks);
